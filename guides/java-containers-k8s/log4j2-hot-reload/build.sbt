...
object Packaging {
  ...
  import cloudflow.sbt.CloudflowKeys.{cloudflowDockerBaseImage, baseDockerInstructions}
  import cloudflow.sbt.CloudflowBasePlugin.UserInImage
  ...
  // we use the same uid as cloudflow does
  // see https://gitlab.prod.build.devtools.smec.services/solutions/retailhub/retailhub-solution-container-images
  private val DockerUserId = UserInImage // "185"
  private val DockerUserId0 = s"$DockerUserId:0"

  object EnvVars {
    val AppHome = "SMEC_RH_APP_HOME"
    val AppConfig = "SMEC_RH_APP_CONFIG_HOME"
  }
  ...
  private object ContainerDirs {
    ...
    val AppConfigPrometheusDir = s"$AppConfigDir/prometheus"
  }
  ...
  object ResourcesDirs {
    ...
    val appConfigPrometheusDir: File = appConfigDir / "prometheus"
  }

  private val envVars = Map(
    EnvVars.AppHome -> ContainerDirs.AppInstallDir,
    EnvVars.AppConfig -> ContainerDirs.AppConfigDir
  )
